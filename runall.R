...
fake_data <- simulate_validatedData(
  ...
  theta = test_theta,
  save_datasets = TRUE, 
  save_masked_datasets = TRUE, directory = here::here("Testing")
)

# This line will take 2-4 minutes to run depending on machine capacity
# If `parallel = FALSE`, disregard warning messages re: rmarginal_autoID return type
# as this function is only a placeholder and is **NOT** evaluated by NIMBLE
sims_output <- run_sims(
  data_list = fake_data$masked_dfs, zeros_list = fake_data$zeros, 

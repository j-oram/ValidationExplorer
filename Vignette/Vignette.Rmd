...
The output from `tune_mcmc` is a list:
...
To decide the number of iterations and warmup for use in a simulation study, we recommend beginning by creating trace plots, which show how the sampled values of a parameter evolve over the course of each Markov chain. To ensure that inferences based on MCMC are reliable, we need to ensure that chains stationary and are mixing well, and that effective sample sizes are sufficiently large to characterize the posterior. Trace plots are especially useful for assessing the first of these. To create a trace plot using the bayesplot package [@bayesplot] for a single parameter, by run the following: 
...
We need to check that chains are stationary and mixing for all parameters. One way to accelerate visual inspection this is through the `regex_pars` argument, as in the following three code blocks. 
...
After making traceplots and zooming in for all parameters, it appears that the guessed warmup value of `r tune_list$min_warmup`output from `tune_mcmc` is reasonable. Even so, we increase our iterations and warmup beyond these minimum values in Section \@ref(runSims) as an extra safeguard against convergence issues. 

#### Examine effective sample size and Gelman-Rubin statistics

As a final step, we examine the effective sample size (out of 10,000 draws) and $\hat{R}$ values:

```{r}
tune_list$convergence_df
```

For all parameters, the bulk and tail effective sample sizes are fairly large, meaning that even with a smaller number of draws, we could expect to characterize both the center and tails of the posteriors for all parameters with these draws. Furthermore, the $\hat{R}$ values for all parameters is near 1. In general, we want values of $\hat{R} \leq 1.1$ for chains to be considered converged. We can double check this on the smaller values
...
These results appear satisfactory, with effective sample sizes in both the tail and bulk of the posterior distributions of more than 250 and $\hat{R}$ near 1. Based on the results of MCMC exploration, it appears that using an MCMC with 2000 iterations with 500 discarded as warmup is likely to produce good results for our simulation study.
...
## Step 5: Fit models to simulated data \label{runSims}
...
The greater detail of `visualize_single_parameter` in Figure \@ref(fig:vizOne) underscores the difference in the number of converged models, shown by the number grey 95% posterior interval for each converged model fit. The visualization also helps clarify that while average interval width is larger than 3, it is not substantially larger in validation scenario 4. This can also be seen using the `plot_width_vs_calls` function (Figure \@ref(fig:plotWidth)), which compares average interval widths (and the IQR for interval widths) across scenarios based on the number of recordings validated. An example is provided below:
...
In Section \@ref(stratEx), we provide further details of the example in the main manuscript, which assumed a stratified-by-species design. In this scheme, the random mechanism is a stratified random sample, with strata formed by the species labels assigned by automated classification software responsible for classifiying each recording to a species. In a stratified-by-species design, the LOVE is the proportion of recordings selected to be validated for each species label.

We repeat the simulation study conducted in the main text in this section, providing greater detail for some of the steps. We assume the same measurable objectives and cost constraints.

## Simulate data

Using the same values for `psi` and `lambda` as defined in Section \@ref(dataSimulation), we simulate data using `simulate_validatedData` with `design_type = "BySpecies"`.
...
**Edit this:** [Note that when specifying a stratified-by-species design as in the main text, `simulate_validatedData` expects that the argument `scenarios` to be a list of proportions corresponding to the possible levels of effort for each species. Additionally, note that `simulate_validatedData` considers all possible combinations of the various levels of effort for each species, meaning that the number of simulated scenarios (and the computation time to run a full simulation study) can grow quickly.]

We can combine the summary object output from `simulate_validatedData` when `design_type = "BySpecies"` with the output of `summarize_n_validated` to understand what the scenarios are and how many recordings are validated under each.

```{r}
summary1 <- sim_data$scenarios_df
summary2 <- summarize_n_validated(sim_data$masked_dfs, scenario_numbers = 1:6)

left_join(summary1, summary2, by = "scenario")
```

Based on these results output, we see that scenario 1 has the lowest overall level of validation effort: species 1-3 have 15% of their recordings validated, species 4 has 10% validated, species 5 has 50%, and species 6 has 25%, yielding around 604 recordings validated in an average dataset. We use one dataset simulated under this scenario to tune the MCMC.
...
Because we set `return_fit = TRUE`, we have draws for each parameter, which allows us to visually inspect the chains for convergence. We set trace plot windows to be wider than the suggested values from `tune_mcmc`, meaning a lower starting value and a higher ending value. Based on these plots, chains appear to be mixing well as no one chain stands out visually.
...
mcmc_trace(fit, regex_pars = "lambda", window = c(0, 2000))
mcmc_trace(fit, regex_pars = "psi", window = c(0, 2000))
mcmc_trace(fit, regex_pars = "theta", window = c(0, 1500))
```
...
As a final step, we examine the effective sample size (out of 10,000 draws):

```{r}
tune_list$n_eff
```

For all parameters, the bulk and tail effective sample sizes are fairly large, meaning that even with a smaller number of draws, we could expect to characterize both the center and tails of the posteriors for all parameters with these draws. We can double check this:

```{r}
# for each chain, extract iterations 501:2000 for all parameters
shortened <- lapply(fit, function(x) x[501:2000,])
...
These results appear satisfactory, with effective sample sizes in both the tail and bulk of the posterior distributions of more than 250. Based on the results of MCMC exploration, it appears that using an MCMC with 2000 iterations with 500 discarded as warmup is likely to produce good results for our simulation study.
...
As a final point, we emphasize that results from any simulation study -- including those produced using the `ValidationExplorer` package -- are conditional on the settings and assumptions of the study. In the case of the count-detection model framework that is implemented in `ValidationExplorer`, the assumptions are

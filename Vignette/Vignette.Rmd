...
## Step 6: Visualize simulations \label{viz}
...
__[Katie, should I go as in depth as in the previous example? It feels a bit strange to show all of the output twice, since they are essentially the same, but maybe we want the sections to stand alone..? Sometimes that can be nice, but I could also imagine it being overly cumbersome.]__
...
Using the same values for `psi`, `lambda` and `Theta` as in Section \@ref(dataSimulation), we simulate data using `simulate_validatedData` with `design_type = "BySpecies"`.

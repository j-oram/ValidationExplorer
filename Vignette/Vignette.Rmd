...
While tuning the mcmc, we found that the first validation scenario - validating a random sample of 5% of recordings from the first visit to each site, required more than 10,000 MCMC iterations to converge. In our experience, this is often an indication that insufficient information is available to estimate one or more model parameter. As a result, we dropped this scenario from consideration. Of the remaining scenarios, `visualize_parameter_group` indicated that scenario 2 -- validating a random sample of 10% of recordings from the first visit to each site -- often led to models that did not converge. Under this scenario, models that did converge exhibited wide posterior intervals for some parameters. Among the remaining scenarios, only inference for $\lambda_3$ did not meet measurable objectives; average posterior interval widths were slightly larger than 3. The sceanario that yielded the narrowest intervals for this parameter was scenario 5, which assumed that 50% of recordings from the first site-visit are randomly selected for validation. Faced with results such as these, monitoring programs must consider whether the deviations from measurable objectives are acceptable, or whether a greater level of effort is required. In our case, we would advocate for an increased level of effort because the number of recordings validated under scenario 5 is substantially below the budget of 4000 recordings. 

Note that with any simulation study results are conditional on the settings and assumptions. In the case of the count-detection model framework, the assumptions are 

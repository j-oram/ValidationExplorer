...
We begin by selecting the parameter values for each species (i.e., assumed occurrence probability and relative activity rate). If estimates exist for the species of interest, one option could be to adopt those. For instance, if we assumed an assemblage comprised of *Eptesicus fuscus* (EPFU)*, Lasiurus cinereus* (LACI)*, Lasinoycteris noctivagans* (LANO), and *Myotis lucifugus* (MYLU), we could use posterior estimates obtained by @stratton22. These are shown in Table \ref{parVals}.
...
With the parameter values chosen, we can Note that the `scenarios` argument is now a vector, in contrast with the proceeding example.
